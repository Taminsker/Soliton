...
/** ********************************************************************** *
 * ITEM_T::DAMPING_PHI_PHI
 * ********************************************************************** *
 * +<PHI(J), chi PHI(I)>
 **/
template <>
void
    SolitonFEItem<ITEM_T::DAMPING_PHI_PHI>::Compute (real_t)
{
    ...
    //    return;
    //  }
    auto FunDamp = [=] (Point * p) -> real_t { return GetDampingCoeffFor (p, mesh, m_tag2Apply, m_c->m_coeff_pen); };
        ...
        for (ul_t i = 0; i < static_cast<ul_t> (numPointsOnCell); ++i)
        {
            ...
            for (ul_t j = 0; j < static_cast<ul_t> (numPointsOnCell); ++j)
            {
                ...
                for (ul_t k = 0; k < quad4Cell->npts; ++k)
                {
                    ...
                    coeff_matrix += weight * 1. / FunDamp (&point_real) * phi_j * phi_i;
