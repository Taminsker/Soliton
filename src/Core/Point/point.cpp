...
Point::Point (real_t a, real_t b, real_t c) :
    x (a), y (b), z (c),
    m_cells ({}),
    m_listNeighbours ({}),
    m_globalIndex (NONE_ID_SELECTED) // Il n'a pas encore d'indice global
...
Point operator* (const Matrix3x3_eig &A, const Point& p)
{
    Point out;
...
void InitPointVector (std::vector<Point*>* vec, ul_t size, Point&& data)
{
    vec->resize (size);
