...
//    }
    ...
    for (int cellId = 0; cellId < numCells; ++cellId)
    {
        ...
        int tagSurrogateCell = listTagsSurrogateCell->vec.at (static_cast<std::size_t>(cellId));

        if (tagSurrogateCell != int(itemThis->GetSolver ()->GetInterTag ()) && itemThis->GetSolver ()->GetInterTag () != INTER::DEFAULT)
            continue;
        ...
        for (std::size_t edgeId = 0; edgeId < numEdgesOnCell; ++edgeId)
        {
            ...
            if (tagPhysicalEdge!= static_cast<int>(itemThis->GetTag2Apply ()) && tagPhysicalEdge != static_cast<int>(PHYS::DEFAULT))
                continue;
