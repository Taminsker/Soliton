...
main (int argc, char * argv [])
{
    ...
    for (SolitonFEContainer * container : list_containers)
    {
        DenseVector * u_0_0 = container->GetQueue (0)->GetNumber (0);
        DenseVector * u_1_0 = container->GetQueue (1)->GetNumber (0);
        DenseVector * u_2_0 = container->GetQueue (2)->GetNumber (0);
    ...
    for (int itTime = 1; itTime < maxIter; ++itTime)
    {
        ...
        for (SolitonFEContainer * container : list_containers)
        {
            ...
            DenseVector * u_0_n = container->GetQueue (0)->GetNumber (0);
            DenseVector * u_1_n = container->GetQueue (1)->GetNumber (0);
            DenseVector * u_2_n = container->GetQueue (2)->GetNumber (0);
            ...
            DenseVector u_2_alpha = alpha * *u_2_n + (1. - alpha) * u_2_np1;
            DenseVector u_1_np1   = *u_1_n + dt * u_2_alpha;
            ...
            DenseVector u_2_beta = 2. * beta * *u_2_n + (1. - 2. * beta) * u_2_np1;
            DenseVector u_0_np1  = *u_0_n + dt * *u_1_n + 0.5 * dt * dt * u_2_beta;
